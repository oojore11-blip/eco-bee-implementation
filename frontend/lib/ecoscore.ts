...
export function calculateEcoScoreFromQuizResponses(
  ...
  Object.keys(boundaryScores).forEach((boundary) => {
    boundaryScores[boundary] = Math.max(
      0,
      Math.min(100, boundaryScores[boundary])
    );
  });

  // Calculate weighted composite score
  const composite = Object.keys(PLANETARY_BOUNDARIES).reduce(
    (sum, boundary) => {
      return (
        sum + boundaryScores[boundary] * PLANETARY_BOUNDARIES[boundary].weight
      );
    },
    0
  );
  ...
  const grade =
    composite <= 30 ? "A" : composite <= 50 ? "B" : composite <= 70 ? "C" : "D";
